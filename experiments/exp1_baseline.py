        ...
        for name, Cls, kwargs, needs_y in algos:
            ...
            if not needs_y:
                ...
                if y_tr.sum() == 0 or (y_tr == 1).all():
                    err = RuntimeError("train set has single class")
                    record_failure(
                        log_path=LOG_PATH,
                        experiment_name="exp1",
                        modality="timeseries",
                        ...
                        error=err,
                        fit_params=kwargs,
                        notes="train set has single class",
                        X_train=X_tr,
                        y_train=y_tr,
                        y_test=y_te,
                    )
            ...
            if _run_one(
                ...
                fit_input, X_te, y_te, X_train=fit_input[0] if isinstance(fit_input, tuple) else fit_input,
                y_train=y_tr,
            ):
                n_ok += 1
    return n_ok, n_total
...
def main():
    ...
    if args.modality in ("timeseries", "all"):
        ...
        ok, total = run_timeseries(TIMESERIES_DATASETS, algos, data_root=args.data_root)
        n_ok += ok
        n_total += total
    ...
    if args.modality in ("graph", "all"):
        ...
        if algos:
            ok, total = run_graph(GRAPH_DATASETS, algos, data_root=args.data_root)
            n_ok += ok
            n_total += total
        else:
